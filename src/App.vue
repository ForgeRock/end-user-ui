  ...
  <div id="app">
    ...
    >
      <div
        ...
        v-if="
          !this.$route.meta.hideToolbar &&
          this.$root.userStore.state.userId !== null
        "
      >
        <ul class="sidebar-nav">
          <li class="sidebar-brand">
            ...
            >
              <img
                src="@/assets/images/horizontal-logo-white.svg"
                :alt="$t('common.form.logo')"
                style="width: 131px"
                class="align-self-center sidebar-brand-logo"
              />
              <img
                src="@/assets/images/vertical-logo-white.svg"
                :alt="$t('common.form.logo')"
                style="height: 28px"
          ...
          <li>
            <router-link :to="{ name: 'Dashboard' }"
              ><i class="fa fa-fw mr-3 fa-tachometer-alt"></i
              >
          ...
          <li>
            <router-link :to="{ name: 'Profile' }"
              ><i class="fa fa-fw mr-3 fa-street-view"></i
              ><span class="sidebar-item-text">{{
                $t("pages.app.profile")
              }}</span></router-link
            >
          ...
          <li
            v-if="
              $root.applicationStore.state.amDataEndpoints &&
              this.$root.userStore.state.internalUser === false
            "
          >
            <router-link :to="{ name: 'Sharing' }"
              ><i class="fa fa-fw mr-3 fa-share"></i
              ><span class="sidebar-item-text">{{
                $t("pages.app.sharing")
              }}</span></router-link
            >
          ...
          <template
            v-for="(access, index) in this.$root.userStore.state.access"
          >
            <li :key="'accessResource' + index">
              <router-link
                :to="{
                  ...
                  meta: { title: 'User' },
                  params: {
                    resourceType: access.privilegePath.split('/')[0],
                    resourceName: access.privilegePath.split('/')[1],
                  },
                }"
                ><i :class="accessIcon(access.icon)"></i
                ><span class="sidebar-item-text">{{
                  access.title
                }}</span></router-link
              >
      ...
      >
        ...
        >
          ...
          <b-navbar-brand class="ml-4" v-if="this.$route.params.resourceName">{{
          ...
          <b-navbar-nav class="ml-auto flex-row">
            ...
            <b-nav-item-dropdown class="fr-main-dropdown" right>
              ...
              <b-dropdown-item
                ...
                :to="{ name: 'Profile' }"
                >{{ $t("pages.app.profile") }}</b-dropdown-item
              >
              <b-dropdown-item
                v-if="this.$root.userStore.state.adminUser"
                href="/admin/"
                >{{ $t("pages.app.admin") }}</b-dropdown-item
              >
      ...
      <template slot="body" slot-scope="props">
          ...
          :class="[
            {
              ...
              'alert-warning': props.item.type == 'warning',
              'alert-danger': props.item.type == 'error',
              'alert-info': props.item.type == 'info',
            },
            'alert',
            'alert-dismissible',
            'd-flex',
        ...
        >
          <div
            :class="[
              {
                'text-success': props.item.type == 'success',
                'text-warning': props.item.type == 'warning',
                'text-danger': props.item.type == 'error',
                'text-info': props.item.type == 'info',
              },
              'alert-icon',
              'mr-3',
              'align-self-top',
          ...
          >
            <i
              :class="[
                {
                  'fa-check-circle-o': props.item.type == 'success',
                  'fa-exclamation-triangle': props.item.type == 'warning',
                  'fa-times-circle': props.item.type == 'error',
                  'fa-info-circle': props.item.type == 'info',
                },
                'fa',
                'fa-lg',
              ]"
