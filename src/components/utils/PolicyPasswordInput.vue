    ...
    <slot v-if="!customInput" name="input-with-validation-panel">
      ...
      >
        <fr-policy-panel
          ...
          :policies="policies"
          :policyFailures="
            showDefaultPolicyFailures ? defaultPolicyFailures : policyFailures
          "
        ></fr-policy-panel>
      </fr-floating-label-input>
    </slot>
...
export default {
        ...
        return [
          {
            ...
            predicate(policyRequirements) {
              return (
                _.includes(policyRequirements, "REQUIRED") &&
                _.includes(policyRequirements, "MIN_LENGTH")
              );
            },
          },
          {
  ...
  watch: {
    failedExcludedPolicies(value) {
      ...
      if (policy) {
        let policyError = this.$t(
            `common.policyValidationMessages.${policy.policyRequirements[0].policyRequirement}`,
            policy.policyRequirements[0].params
          ),
          errorMessage = `${this.$t(
            `common.policyValidationMessages.policyValidationFailed`,
  ...
  computed: {
    ...
    customInput() {
      return !_.isUndefined(this.$slots["custom-input"]);
    },
  },
  methods: {
    /**
    ...
    toPolicyNames(failedPolicySet) {
      let failedPolicyRequirements =
          failedPolicySet.failedPolicyRequirements || [],
        policyNames = failedPolicyRequirements
          .filter(this.isPasswordPolicyItem("property"))
          .map((failedPolicyDefinition) => {
            return _.at(failedPolicyDefinition, [
              "policyRequirements[0].policyRequirement",
            ]);
          });
    ...
    makeExclusions(policyRequirementSet) {
      let policyRequirements, policies;
      ...
      const rejectPolicy = (requirement) => {
        return _.reject(
          policies,
          (policy) => _.first(policy.policyRequirements) === requirement
        );
      };
      ...
      return Object.assign({}, policyRequirementSet, {
        policyRequirements,
        policies,
      });
    },
    ...
    getAction() {
      return this.policyApi.match("selfservice")
        ? "validateObject"
        : "validateProperty";
    },
  },
  created() {
        ...
        return policyService
                ...
                (failedPolicyRequirement) => {
                  if (failedPolicyRequirement.property === "password") {
                    ...
                    if (
                      _.includes(
                        this.exclude,
                        failedPolicyRequirement.policyRequirements[0]
                          .policyRequirement
                      )
                    ) {
          ...
          .catch(() => {
            ...
            this.displayNotification(
              "error",
              this.$t("common.policyValidationMessages.policyServiceError")
            );
          });
      }.bind(this);
    ...
    /* istanbul ignore next */
    policyService
      .get(`/policy/${this.policyApi}`)
      .then(({ data }) =>
        _.head(data.properties.filter(this.isPasswordPolicyItem("name")))
      )
      ...
      .catch(() => {
        ...
        this.displayNotification(
          "error",
          this.$t(
            `common.policyValidationMessages.policyServiceError.${this.policyApi}`
          )
        );
