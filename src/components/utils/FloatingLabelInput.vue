  ...
  <div class="mb-3">
    ...
    >
      <input
        ...
        :class="[
          {
            polyfillPlaceholder: floatLabels,
            'is-invalid': errors.has(fieldName) && showErrorState,
          },
          'form-control',
        ]"
        :autofocus="autofocus"
      ...
      <div v-if="reveal" class="input-group-append">
        <button @click="revealText" class="btn btn-secondary" type="button">
          <i :class="[{ 'fa-eye-slash': !show }, { 'fa-eye': show }, 'fa']"></i>
        </button>
      </div>
    ...
    _.delay(
      _.bind(() => {
        if (navigator.userAgent.indexOf("Edge") >= 0) {
          if (document.getElementById(`${this.id}`) && document.getElementById(`${this.id}`).value.length) {
            this.floatLabels = true;
            this.inputValue = document.getElementById(`${this.id}`).value;
          }
        } else if (navigator.userAgent.indexOf("Chrome") >= 0) {
          if (document.querySelectorAll(`#${this.id}:-webkit-autofill`) && document.querySelectorAll(`#${this.id}:-webkit-autofill`).length > 0) {
            this.floatLabels = true;
          }
        }
