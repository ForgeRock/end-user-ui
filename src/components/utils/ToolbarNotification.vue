  ...
  >
    ...
    <b-dropdown-header class="border-bottom py-3">
      <div class="fr-notification-header">
        <span
          >{{ $t("pages.app.notifications.title") }} ({{
        ...
        <a
          ...
          class="float-right"
          href="#"
          >{{ $t("pages.app.notifications.clearAll") }}</a
        >
      </div>
    </b-dropdown-header>
    ...
    <template v-if="notifications.length > 0">
      <div class="scrollbox" is="transition-group" name="notification-list">
        <div
          ...
          :class="[
            { 'border-bottom': index + 1 < notifications.length },
            'dropdown-item',
            'py-3',
  ...
  methods: {
    ...
    clearAll() {
      ...
      this.getRequestService()
        .post(
          `/notification?_action=deleteNotificationsForTarget&target=${target}`
        )
        .then(() => {
          this.displayNotification(
            "success",
            this.$t("pages.app.notifications.removedAll")
          );
        ...
        .catch(() => {
          this.displayNotification(
            "error",
            this.$t("pages.app.notifications.failedToClear")
          );
        });
    },
    clearOne(index) {
      ...
      this.getRequestService()
        ...
        .then(() => {
          this.displayNotification(
            "success",
            this.$t("pages.app.notifications.removed")
          );
        ...
        .catch(() => {
          this.displayNotification(
            "error",
            this.$t("pages.app.notifications.failedToRemove")
          );
        });
    },
    loadData() {
      ...
      if (!_.isNull(this.$root.userStore.state.userId)) {
        this.getRequestService()
          .get(
            `/${this.$root.userStore.state.managedResource}/${this.$root.userStore.state.userId}?_fields=_notifications/*`
          )
          .then(({ data }) => {
