  ...
  methods: {
    completeTask({ id, formData }) {
      ...
      return this.workflowService
        .post(`/workflow/taskinstance/${id}?_action=complete`, formData, {
          headers: { Accept: "application/json, text/javascript, */*; q=0.01" },
        })
        .then(() => {
          this.displayNotification(
            "success",
            this.$t("pages.workflow.taskSuccessfullyCompleted")
          );
        ...
        .catch((error) => {
          if (error.response.data.code === 403) {
            this.displayNotification(
              "error",
              this.$t("pages.workflow.taskNoLongerAvailable", {
                taskName: this.assignedTasks[id].name,
              })
            );
            this.$delete(this.assignedTasks, id);
            this.loadTasks();
    ...
    updateAssignment({ id, task, assignee }) {
      ...
      return this.workflowService
        .put(
          `/workflow/taskinstance/${task._id}`,
          { assignee },
          { headers: { "If-Match": '"*"' } }
        )
        .then(() => {
          this.displayNotification(
            "success",
            this.$t("pages.workflow.assignmentSuccess", {
              taskName: task.name,
              assignee,
            })
          );
        ...
        .catch((error) => {
          if (error.response.data.code === 403) {
            this.displayNotification(
              "error",
              this.$t("pages.workflow.taskNoLongerAvailable", {
                taskName: this.availableTasks[id].name,
              })
            );
            this.$delete(this.availableTasks, id);
            this.loadTasks();
    ...
    startProcess(payload) {
      /* istanbul ignore next */
      return this.workflowService
        .post("/workflow/processinstance/?_action=create", payload)
        .then(() => {
          this.displayNotification(
            "success",
            this.$t("pages.workflow.processStartSuccessMessage")
          );
    ...
    loadProcesses() {
      ...
      return (
        this.workflowService
          .get("/endpoint/getprocessesforuser")
          ...
          .then((processes) => {
            return axios.all(
              processes.map((process) => {
                let fetchProcessDefinition = () =>
                    this.workflowService.get(
                      `/workflow/processdefinition/${process._id}`
                    ),
                  processDefinition = null;
                return _.merge(process, {
                  fetchProcessDefinition,
                  processDefinition,
                });
              })
    ...
    toTasks(taskGroup, { data }) {
      ...
      _.each(tasks, (taskObj, id) => {
        let name = taskObj.name,
          ...
          fetchProcessDefinition = () => {
            return this.workflowService.get(
              `/workflow/processdefinition/${task.processDefinitionId}`
            );
          };
    ...
    loadData() {
      ...
      this.loadProcesses() // Need to load processes first so process definitions are available to tasks when loaded
        .then(() =>
          axios.all([
            this.loadTasks({ groupName: "assignedTasks" }),
            this.loadTasks({ groupName: "availableTasks" }),
          ])
        )
        .catch((error) => {
          this.displayNotification("error", error.response.data.message);
