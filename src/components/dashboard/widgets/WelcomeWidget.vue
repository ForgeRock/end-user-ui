<template>
  <b-jumbotron class="text-center">
    <template slot="header">
      ...
      />
      <div>
        {{ $t("pages.dashboard.widgets.welcome.greeting") }}, {{ fullName }}
      </div>
    </template>
    <template slot="lead">
      <div>
...
export default {
  ...
  methods: {
    openProfile() {
      this.$router.push({
        name: "Profile",
        params: { openProfile: !this.$root.userStore.state.internalUser },
      });
    },
  },
  computed: {
    fullName() {
      ...
      if (
        this.userDetails.givenName.length > 0 ||
        this.userDetails.sn.length > 0
      ) {
        fullName = _.startCase(
          this.userDetails.givenName + " " + this.userDetails.sn
        );
      } else {
