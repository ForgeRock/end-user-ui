          ...
          <div class="col-sm-10">
            <div class="d-flex">
              ...
              <b-button
                ...
                @click="assignTask"
                >{{ $t("pages.workflow.assign") }}</b-button
              >
            </div>
          </div>
        </div>
            ...
            <b-card>
              <dl class="row m-0">
                <template v-for="(detail, index) in taskDetailsList">
                  <dt :key="`taskname-${index}-${uniqueId}`" class="col-6">
                    {{ detail.name }}
                  </dt>
                  <dd
...
export default {
  ...
  computed: {
    assigneeOptions() {
      ...
      if (!_.isEmpty(this.taskDefinition.task.usersToAssign)) {
        return this.taskDefinition.task.usersToAssign.map(
          ({ username, displayableName }) => {
            let value = username,
              text =
                username === loggedUserName
                  ? this.$t("pages.workflow.me")
                  : displayableName;
            return { value, text };
          }
        );
  ...
  methods: {
    assignTask() {
      this.$emit("assignTask", {
        id: this.taskDefinition.task._id,
  ...
  watch: {
    shown(val) {
      if (
        val &&
        (_.isNull(this.taskDefinition.process.processDefinition) ||
          _.isUndefined(this.taskDefinition.process.processDefinition)) &&
        this.taskDetailsList.length === 0
      ) {
        this.getRequestService()
          .get(
            `/workflow/processdefinition/${this.taskDefinition.task.processDefinitionId}`
          )
          .then((processDetails) => {
            this.taskDetailsList = this.generateDisplayDetails(
              processDetails.data.formProperties,
              this.taskDefinition.task.variables
            );
          });
      } else if (this.taskDetailsList.length === 0) {
        this.taskDetailsList = this.generateDisplayDetails(
          this.taskDefinition.process.processDefinition.formProperties,
          this.taskDefinition.task.variables
        );
      }
    },
