<template>
  <transition name="fade" mode="out-in" duration="250">
    <component
      v-if="processDefinition !== null && showGenericTask === false"
      :is="taskForm"
      @submit="submit"
      @cancel="cancel"
    ...
    ></component>
    <GenericTask
      v-else-if="showGenericTask === true && processDefinition !== null"
      :variables="taskInstance.task.variables"
...
export default {
  ...
  data() {
    ...
    if (
      this.taskInstance.process &&
      this.taskInstance.process.processDefinition
    ) {
  ...
  computed: {
    taskForm() {
      const formGenerationTemplate =
          this.taskInstance.task.taskDefinition.formGenerationTemplate,
    ...
    shown(val) {
      if (val && _.isNull(this.processDefinition)) {
        this.getRequestService()
          .get(
            `/workflow/processdefinition/${this.taskInstance.task.processDefinitionId}`
          )
          .then((processDetails) => {
