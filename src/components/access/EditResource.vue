<template>
  <b-container>
    <b-row>
      <b-col>
        <div class="d-sm-flex my-4">
          <div class="media">
            <div
              class="rounded-circle fr-resource-circle text-light bg-primary mr-4"
            >
              <i :class="setIcon" />
            </div>
          ...
          >
            <b-dropdown
              ...
              class="m-md-2"
            >
              <b-dropdown-item v-if="canChangePassword" v-b-modal.resetModal>{{
                $t("pages.access.resetPassword")
              }}</b-dropdown-item>
              <b-dropdown-item v-if="canDelete" v-b-modal.deleteModal
                >{{ $t("common.form.delete") }} {{ this.name }}</b-dropdown-item
              >
            </b-dropdown>
          </div>
        </div>
    ...
    <b-tabs flex-column flex-sm-row>
        ...
        <b-card>
          <template v-if="displayProperties.length > 0">
              ...
              <template v-if="!field.isReadOnly">
                ...
                >
                  <b-form-input
                    ...
                    type="text"
                    :class="[
                      { 'is-invalid': errors.has(`mainEdit.${field.key}`) },
                    ]"
                ...
                >
                  <div class="form-row">
                    <label class="col-form-label col-sm-3" :for="field.title">{{
                      field.title
                    }}</label>
            ...
            <div v-if="!disableSaveButton" class="float-right mt-4">
              <b-btn type="button" @click="saveResource" variant="primary">{{
                $t("common.form.save")
              }}</b-btn>
            </div>
          </template>
    ...
    >
      <div>{{ $t("pages.access.deleteConfirm") }} {{ this.name }}?</div>

      <div slot="modal-footer" class="w-100">
        <div class="float-right">
          <b-btn type="button" variant="danger" @click="deleteResource">{{
    ...
    >
      <fr-password-policy-input
        :policyApi="`${resource}/${name}/policyTest`"
      ...
      <div slot="modal-footer" class="w-100">
        <div class="float-right">
          <b-btn type="button" variant="primary" @click="savePassword">{{
            $t("common.form.save")
          }}</b-btn>
        </div>
      </div>
  ...
  methods: {
    loadData() {
      ...
      axios
        .all([
          ...
          idmInstance.get(`${this.resource}/${this.name}/${this.id}`),
        ])
        .then(
          axios.spread((schema, privilege, resourceDetails) => {
    ...
    generateDisplay(schema, privilege, resourceDetails) {
      ...
      if (privilege.VIEW.allowed) {
        ...
        _.each(
          this.mergePrivilegeProperties(privilege, schema),
          (createPriv) => {
            ...
            if (
              (_.toLower(createPriv.attribute) === "username" ||
                _.toLower(createPriv.attribute) === "name") &&
              this.displayNameField.length === 0
            ) {
            ...
            if (
              (_.toLower(createPriv.attribute) === "title" ||
                _.toLower(createPriv.attribute) === "email" ||
                _.toLower(createPriv.attribute) === "type" ||
                _.toLower(createPriv.attribute) === "mail") &&
    ...
    deleteResource() {
      ...
      /* istanbul ignore next */
      idmInstance
        .delete(`${this.resource}/${this.name}/${this.id}`)
        .then(() => {
          this.displayNotification(
            "success",
            this.$t("pages.access.deleteResource")
          );
    ...
    saveResource() {
      ...
      this.$validator.validate("mainEdit.*").then((valid) => {
        if (valid) {
          let saveData = this.generateUpdatePatch(
            _.clone(this.oldFormFields),
            _.clone(this.formFields)
          );

          idmInstance
            .patch(`${this.resource}/${this.name}/${this.id}`, saveData)
            .then(
              () => {
                this.oldFormFields = _.clone(this.formFields);
                this.displayNotification(
                  "success",
                  this.$t("pages.access.successEdited", {
                    resource: _.capitalize(this.name),
                  })
                );
              },
              (error) => {
                ...
                if (generatedErrors.length > 0) {
                  ...
                  this.displayNotification(
                    "error",
                    this.$t("pages.access.invalidEdit")
                  );
                }
              }
            );
          ...
          idmInstance
            ...
            .then(
              () => {
                this.displayNotification(
                  "success",
                  this.$t("pages.access.successSavePassword")
                );
              },
              () => {
                this.displayNotification(
                  "error",
                  this.$t("pages.access.failedSavePassword")
                );
              }
            );
        } else {
    ...
    mergePrivilegeProperties(privilege, schema) {
      ...
      _.each(schema.order, (schemaPropName) => {
        let canView = _.indexOf(privilege.VIEW.properties, schemaPropName) > -1,
          canUpdate =
            _.indexOf(privilege.UPDATE.properties, schemaPropName) > -1,
