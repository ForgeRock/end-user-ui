        ...
        <b-row>
          <b-col md="3" class="my-1">
            <b-btn
              ...
              ><i class="fa fa-plus mr-3"></i>{{ $t("common.form.new") }}
              {{ this.name }}</b-btn
            >
          </b-col>
        ...
        >
          <template v-slot:table-busy>
            <div class="text-center p-5">
              <b-spinner
                class="align-middle spinner-large text-primary my-4"
              ></b-spinner>
      ...
      <div v-if="!isLoading" class="card-footer py-2">
        <nav aria-label="Page navigation example">
          <ul class="pagination justify-content-center mb-0">
            ...
            <li
              @click.prevent="
                currentPage === 1 ? '' : paginationChange(currentPage - 1)
              "
              :class="[{ disabled: currentPage === 1 }, 'page-item']"
            >
...
export default {
  ...
  watch: {
    gridData() {
      ...
      this.columns = this.columns.map((col) => {
        if (col.key !== "actions") {
          col.sortable =
            !this.queryThreshold || this.filter.length >= this.queryThreshold;
        }
  ...
  computed: {
    searchPlaceholder() {
      if (this.queryThreshold) {
        return this.$t("pages.access.typeAtLeastAndEnterToSearch", {
          numChars: this.queryThreshold,
        });
      }
  ...
  methods: {
    loadData() {
      const idmInstance = this.getRequestService(),
        managedObjectsSettings =
          this.$root.applicationStore.state.managedObjectsSettings;

      if (managedObjectsSettings && managedObjectsSettings[this.name]) {
        this.queryThreshold =
          managedObjectsSettings[this.name].minimumUIFilterLength;
      }
          ...
          axios.spread((schema, privilege) => {
            if (privilege.data.VIEW.allowed) {
              ...
              _.each(privilege.data.VIEW.properties, (readProp) => {
                if (
                  ...
                  _.isUndefined(schema.data.properties[readProp].encryption)
                ) {
                  this.columns.push({
                    key: readProp,
            ...
            if (privilege.data.CREATE.allowed) {
              ...
              _.each(privilege.data.CREATE.properties, (createProp) => {
                if (
                  schema.data.properties[createProp].type === "string" ||
                  schema.data.properties[createProp].type === "number" ||
                  schema.data.properties[createProp].type === "boolean"
                ) {
                  schema.data.properties[createProp].key = createProp;

                  _.each(schema.data.required, (requiredKey) => {
                    if (
                      requiredKey === schema.data.properties[createProp].key
                    ) {
                      schema.data.properties[createProp].required = true;
                    }
                  });

                  this.createProperties.push(
                    schema.data.properties[createProp]
                  );
                }
              });
            }
    ...
    loadGrid(filter, fields, sortField, page) {
      ...
      /* istanbul ignore next */
      idmInstance
        .get(this.buildGridUrl(filter, fields, sortField, page))
        .then((resourceData) => {
    ...
    sortingChanged(sort) {
      ...
      this.loadGrid(
        ...
        this.displayFields,
        this.calculateSort(sort.sortDesc, sort.sortBy),
        1
      );
    },
    paginationChange(page) {
      ...
      this.loadGrid(
        ...
        this.displayFields,
        this.calculateSort(this.sortDesc, this.sortBy),
        page
      );
    },
    search() {
      ...
      if (
        !this.queryThreshold ||
        !this.filter.length ||
        this.filter.length >= this.queryThreshold
      ) {
        this.loadGrid(
          this.generateSearch(
            this.filter,
    ...
    resourceClicked(item) {
      if (this.userCanUpdate) {
        ...
        this.displayNotification(
          "error",
          this.$t("pages.access.unableToEditResource", { resource: this.name })
        );
      }
    },
    plural(txt) {
