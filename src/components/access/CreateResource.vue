  ...
  >
    <div slot="modal-header" class="d-flex w-100 h-100">
      <h5 class="modal-title align-self-center text-center">
        {{ $t("common.form.new") }} {{ resourceName }}
      </h5>
        ...
        >
          <template v-for="(field, index) in createProperties">
            <b-form-group
              ...
              "
            >
              <label v-if="field.title" class="float-left" :for="field.title">{{
                field.title
              }}</label>
              <label v-else class="float-left" :for="field.key">{{
                field.key
              }}</label>
              ...
              <input
                ...
                type="text"
                :class="[
                  { 'is-invalid': errors.has(field.key) },
                  'form-control',
                ]"
          ...
          <fr-password-policy-input
            v-if="passwordCheck"
            :policyApi="`${resourceType}/${resourceName}/policyTest`"
    ...
    saveForm() {
      ...
      this.$validator.validateAll().then((valid) => {
        if (valid) {
          ...
          idmInstance
            .post(
              `${this.resourceType}/${this.resourceName}?_action=create`,
              saveData
            )
            .then(
              () => {
                ...
                this.displayNotification(
                  "success",
                  this.$t("pages.access.successCreate", {
                    resource: _.capitalize(this.resourceName),
                  })
                );
              },
              (error) => {
                ...
                if (generatedErrors.length > 0) {
                  ...
                  this.displayNotification(
                    "error",
                    this.$t("pages.access.invalidCreate")
                  );
                }
              }
            );
