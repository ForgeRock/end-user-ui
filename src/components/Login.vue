  ...
  <fr-center-card :showLogo="true">
    ...
    <b-card-body slot="center-card-body">
      <transition name="slide-fade">
        <div
          v-if="wrongPasswordSubmitted"
          class="alert alert-danger mb-4 text-left"
          role="alert"
        >
          {{ $t("pages.login.badPassword") }}
          <span v-if="this.$root.applicationStore.state.passwordReset"
            ><br />{{ $t("pages.login.weCanHelp") }}
            <router-link
              v-if="this.$root.applicationStore.state.passwordReset"
              action=""
              :to="{ name: 'PasswordReset' }"
              >{{ $t("pages.login.recoverPassword") }}</router-link
            ></span
          >
          <span v-if="this.$root.applicationStore.state.usernameRecovery"
            ><br />{{ $t("pages.login.weCanHelp") }}
            <router-link action="" :to="{ name: 'ForgotUsername' }">{{
              $t("pages.login.recoverUsername")
            }}</router-link></span
          >
        </div>
      </transition>
      ...
      <p class="text-center">
        <router-link
          v-if="this.$root.applicationStore.state.usernameRecovery"
          action=""
          to="forgotusername"
          >{{ $t("pages.login.forgotUsername") }}</router-link
        >
        <span
          v-if="
            this.$root.applicationStore.state.usernameRecovery &&
            this.$root.applicationStore.state.passwordReset
          "
          >·</span
        >
        <router-link
          v-if="this.$root.applicationStore.state.passwordReset"
          to="passwordreset"
          >{{ $t("pages.login.forgotPassword") }}</router-link
        >
      </p>
    </b-card-body>
    <template v-if="this.$root.applicationStore.state.registration">
      <b-card-footer slot="center-card-footer" class="fr-footer-bottom">
        ...
        <router-link action="" :to="{ name: 'Registration' }">{{
      ...
      idmInstance.post("/authentication?_action=logout").then(() => {
        loginServiceInstance
          ...
          .then((userDetails) => {
            ...
            this.$root.userStore.setUserIdAction(
              userDetails.data.authorization.id
            );
            this.$root.userStore.setManagedResourceAction(
              userDetails.data.authorization.component
            );
            this.$root.userStore.setRolesAction(
              userDetails.data.authorization.roles
            );
            // Check for progressive profiling.
            this.progressiveProfileCheck(userDetails, () => {
              axios
                .all([
                  loginServiceInstance.get(
                  ...
                  this.getSchema(userDetails.data.authorization.component),
                ])
                .then(
