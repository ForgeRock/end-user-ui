<template>
  <fr-center-card :showLogo="true" v-if="showForm">
    ...
    <b-card-footer
      slot="center-card-footer"
      v-if="selfServiceDetails !== null && selfServiceDetails.canSkip"
    >
      ...
      if (_.isEmpty(details.requirements) && details.tag === "initial") {
        ...
        if (
          ...
          (_.has(details.requirements, "attributes") &&
            _.filter(details.requirements.attributes, { isRequired: true })
              .length >= 1)
        ) {
