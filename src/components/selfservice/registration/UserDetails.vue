<template>
  <b-form>
    ...
    >
      ...
      >
        <label class="mr-auto" :for="property.title">{{
          property.title
        }}</label>
...
export default {
  ...
  data() {
    ...
    if (
      this.selfServiceDetails.requirements &&
      this.selfServiceDetails.requirements.registrationProperties
    ) {
      data.userDetails =
        this.selfServiceDetails.requirements.registrationProperties.properties;

      if (
        _.has(
          this.selfServiceDetails,
          "requirements.properties.user.default"
        ) &&
        !_.isUndefined(
          this.selfServiceDetails,
          "requirements.properties.user.default"
        ...
        _.each(
          this.selfServiceDetails.requirements.properties.user.default,
          (value, key) => {
      ...
      _.each(
        this.selfServiceDetails.requirements.registrationProperties.required,
        (prop) => {
    ...
    if (
      this.selfServiceDetails.requirements &&
      this.selfServiceDetails.requirements.registrationPreferences
    ) {
      ...
      this.isValid().then((valid) => {
        if (valid) {
          ...
          this.displayNotification(
            "error",
            this.$t("pages.selfservice.registration.pleaseComplete")
          );
        }
      });
    },
