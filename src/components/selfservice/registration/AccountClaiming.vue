  ...
  <fr-center-card :showLogo="true" v-else>
    <div slot="center-card-header">
      ...
      <span
        ...
        v-html="
          $t('pages.selfservice.accountClaiming.passwordDesc', {
            account: mail,
          })
        "
      ></span>
    ...
    <b-card-body slot="center-card-body">
      ...
      <fr-social-buttons
        ...
        :filterProvidersObjects="providersObjects"
        signin
      ></fr-social-buttons>
...
export default {
  ...
  mounted() {
    if (localStorage.getItem("accountClaimingToken")) {
      let accountClaimingToken = atob(
        localStorage.getItem("accountClaimingToken")
      );
    ...
    socialDescription() {
      ...
      _.each(this.providers, (provider, index) => {
        if (numProviders === 1) {
          ...
          providerList += ` ${this.$t(
            "pages.selfservice.social.or"
          )} ${_.upperFirst(provider)}`;
        }
      });

      return this.$t("pages.selfservice.accountClaiming.socialDesc", {
  ...
  methods: {
    ...
    setChildComponent(type, details) {
      ...
      if (type === "parameters") {
        ...
        details.tag === "verifyProfile"
      ) {
        this.mail = _.has(details, "requirements.mail")
          ? details.requirements.mail
          : "";
        ...
        if (details.requirements.error) {
          ...
          if (this.clientToken && this.originalToken) {
            ...
            this.providers = _.map(
              details.requirements.definitions.providers.items.oneOf,
              "provider"
            );
            this.providersObjects =
              details.requirements.definitions.providers.items.oneOf;
            this.socialVerification = true;
          }
        }
      } else if (
        (type === "localAutoLogin" || type === "openAMAutoLogin") &&
        _.isUndefined(details.additions.claimedProfile)
      ) {
        ...
        const socialLoginInstance = this.getRequestService({
          headers: {
            ...
            "X-OpenIDM-DataStoreToken":
              sessionStorage.getItem("amSocialToken") || this.clientToken,
        ...
        socialLoginInstance
          ...
          .then((userDetails) => {
            ...
            this.progressiveProfileCheck(userDetails, () => {
              if (
                _.has(details, "additions.successUrl") &&
                !_.isEmpty(details.additions.successUrl)
              ) {
              ...
              this.displayNotification(
                "success",
                this.$t("pages.selfservice.accountClaiming.linked")
              );
            });
          })
          .catch((error) => {
        ...
        this.displayNotification(
          "error",
          this.$t("pages.selfservice.accountClaiming.error")
        );
      }
    },
  },
