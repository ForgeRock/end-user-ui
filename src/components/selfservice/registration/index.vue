<template>
  <div class="h-100">
    ...
    <div v-show="showSelfService">
      <fr-center-card
        :showLogo="true"
        v-if="selfServiceType !== 'localAutoLogin'"
      >
        <div slot="center-card-header">
          <h2 v-show="title.length > 0" class="h2">{{ title }}</h2>
          <p v-show="subtitle.length > 0" class="text-center mb-0">
            {{ subtitle }}
          </p>
        </div>
        ...
        <b-card-footer slot="center-card-footer">
          ...
          <router-link action="" :to="{ name: 'Login' }">{{
  ...
  computed: {
    title() {
      if (_.includes(customTitleComponents, this.selfServiceType)) {
        return this.$t(
          `pages.selfservice.registration.stageTitle.${this.selfServiceType}`
        );
      } else {
    ...
    subtitle() {
      if (_.includes(customTitleComponents, this.selfServiceType)) {
        return this.$t(
          `pages.selfservice.registration.stageSubtitle.${this.selfServiceType}`
        );
      } else {
  ...
  methods: {
    setChildComponent(type, details) {
      ...
      if (type === "parameters") {
        ...
        if (
          _.has(details, "additions.oauthLogin") &&
          details.additions.oauthLogin
        ) {
          /* istanbul ignore next */
          ...
          socialLoginInstance
            ...
            .then((userDetails) => {
              this.displayNotification(
                "success",
                this.$t("pages.selfservice.registration.createdAccount")
              );

              if (
                details.additions.successUrl &&
                details.additions.successUrl.length > 0
              ) {
          ...
          this.autoLogin(
            details.additions.credentialJwt,
            details.additions.successUrl
          );
        }
