  ...
  <div>
    ...
    >
      ...
      <fr-floating-label-input
        ...
        :validateRules="{ required: true, unique_question: getDuplicates(key) }"
      ></fr-floating-label-input>
...
export default {
  ...
  computed: {
    predefinedQuestionOptions() {
      return _.map(this.kba.questions, (question) => {
        let value = question.id,
          text =
            question.question[this.$i18n.locale] ||
            question.question[this.$i18n.fallbackLocale],
          disabled = !_.isUndefined(
            _.find(this.answers, { questionId: question.id })
          );
    ...
    options() {
      ...
      return _.concat(
        this.predefinedQuestionOptions,
        placeholder,
        customQuestionOption
      );
    },
    ...
    getData() {
      return {
        kba: this.answers.map((answer) => {
          return _.omit(
            answer,
            answer.questionId === this.customIndex
              ? "questionId"
              : "customQuestion"
          );
        }),
