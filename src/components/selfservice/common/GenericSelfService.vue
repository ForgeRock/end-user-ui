<template>
  <div>
    <template v-for="(field, index) in displayDetails">
      <b-form-group
        :label="field.key | startCase"
        label-for="field.key"
        horizontal
        :key="'genericField' + index"
      ...
      >
        <div class="form-row">
          <label class="col-form-label col-sm-3" :for="field.key">{{
            field.key | startCase
          }}</label>
          <div class="mr-auto">
            <b-form-checkbox
...
export default {
  ...
  data() {
    ...
    if (
      ...
      this.selfServiceDetails.requirements.properties
    ) {
