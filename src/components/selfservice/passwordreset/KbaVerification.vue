<template>
  <b-form @keyup.enter="save" @submit.prevent>
    ...
    <div
      ...
      :key="key"
    >
      <label class="col-form-label pt-0 pb-2" :for="key">{{
        questionText[key]
      }}</label>
    ...
    <b-button @click="save" :block="true" size="lg" variant="primary">
      {{
        $tc(
          "common.user.kba.submitAnswers",
          selfServiceDetails.requirements.required.length
        )
      }}
    </b-button>
  </b-form>
    ...
    return {
      questionText: _.mapValues(properties, (value) => {
        if (_.has(value, "systemQuestion")) {
          let question =
            value.systemQuestion[locale] ||
            value.systemQuestion[fallbackLocale];
          return question;
        }
