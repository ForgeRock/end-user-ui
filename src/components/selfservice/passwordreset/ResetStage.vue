<template>
  ...
  <div v-else-if="typeof selfServiceDetails.error !== 'string'">
    <b-form @keyup.enter="save" @submit.prevent>
      <fr-policy-password-input
        :defaultPolicyFailures="defaultPolicyFailures"
        policyApi="selfservice/reset"
        v-model="password"
        name="password"
  ...
  <div v-else>
    ...
    <div class="mt-2">
      <a href="#/passwordreset" v-on:click="reloadPage">{{
    ...
    selfServiceDetails: {
      handler(val) {
        ...
        if (has(val, "requirements.error.detail.failedPolicyRequirements")) {
          let failedPolicy = find(
            val.requirements.error.detail.failedPolicyRequirements,
            { property: "password" }
          );

          if (failedPolicy && failedPolicy.policyRequirements) {
            let policyError = this.$t(
                `common.policyValidationMessages.${failedPolicy.policyRequirements[0].policyRequirement}`,
                failedPolicy.policyRequirements[0].params
              ),
              errorMessage = `${this.$t(
                `common.policyValidationMessages.policyValidationFailed`,
                { property: failedPolicy.property }
              )}: ${policyError}`;

            /* istanbul ignore next */
            this.displayNotification("error", errorMessage);

            this.defaultPolicyFailures = map(
              failedPolicy.policyRequirements,
              "policyRequirement"
            );
          }
        }
