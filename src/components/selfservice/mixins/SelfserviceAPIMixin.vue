...
export default {
  ...
  methods: {
    loadData() {
      ...
      /* istanbul ignore next */
      selfServiceInstance
        .get(`/selfservice/${this.apiType}`)
        .then((selfServiceDetails) => {
          this.setChildComponent(
            selfServiceDetails.data.type,
            selfServiceDetails.data
          );
        })
        .catch((error) => {
          /*
    ...
    advanceStage(data, noSessionFalse) {
      ...
      if (_.has(data, "returnParams")) {
        ...
        const returnParams = new URLSearchParams(
            decodeURIComponent(data.returnParams)
          ),
          redirectUriOverride = returnParams.get("goto");
        localStorage.setItem(
          "redirectUriOverride",
          btoa(redirectUriOverride)
        );
      }
      /* istanbul ignore next */
      selfServiceInstance
        .post(
          `/selfservice/${this.apiType}?_action=submitRequirements`,
          saveData
        )
        .then((selfServiceDetails) => {
          this.setChildComponent(
    ...
    parseQueryParams(queryParams) {
      ...
      */
      if (!queryParams.match("returnParams")) {
        ...
        return {
          returnParams: _.last(
            decodeURIComponent(queryParams).split("returnParams=")
          ),
        };
      }
    },
