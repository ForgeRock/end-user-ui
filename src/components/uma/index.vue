    ...
    <template v-if="requestsLoaded">
      <b-tabs class="mt-4" v-if="resources.length > 0" @click="testForReload">
        ...
        <b-tab title="Requests" v-if="requests.length > 0">
          <template slot="title">
            {{ $t("pages.uma.notifications.requests") }}
            <b-badge pill variant="danger">{{ requests.length }}</b-badge>
          </template>
          <fr-requests
            :requests="requests"
  ...
  methods: {
    ...
    getResources() {
      ...
      let userName = this.$root.userStore.state.userName,
        ...
        url =
          this.amDataEndpoints.baseUrl +
          userName +
          this.amDataEndpoints.resourceSet +
          query;
    ...
    getActivity() {
      ...
      let userName = this.$root.userStore.state.userName,
        ...
        url =
          this.amDataEndpoints.baseUrl +
          userName +
          this.amDataEndpoints.auditHistory +
          query;
    ...
    modifyResource(resourceId, payload, config = {}) {
      let successMsg = config.unshare
          ? this.$t("common.user.sharing.unshareSuccess")
          : this.$t("common.user.sharing.modifySuccess"),
      ...
      selfServiceInstance
        .put(url, payload, { withCredentials: true, headers })
        .then(() => {
          if (config.onSuccess) {
            config.onSuccess();
          }
    ...
    finalizeResourceAccess(id, action, config = {}) {
      ...
      let userName = this.$root.userStore.state.userName,
        successMsg =
          action === "approve"
            ? this.$t("common.user.sharing.requestAllowedSuccess")
            : this.$t("common.user.sharing.requestDeniedSuccess"),
        selfServiceInstance = this.getRequestService(),
