<template>
  ...
  >
    <div slot="modal-header" class="d-flex w-100 h-100">
      <div class="media">
        ...
        <div class="media-body align-self-center">
          ...
          <small class="text-muted" v-if="!resource.policy">{{
          ...
          <small
            ...
            >{{
              $t("pages.uma.resources.sharedWithPeople", {
                numberOf: resource.policy.permissions.length,
              })
            }}</small
          >
      ...
      <button type="button" aria-label="Close" class="close" @click="hideModal">
    ...
    <div class="form-group">
      <b-input-group>
        ...
        <b-dropdown
          ...
          slot="append"
        >
          <form class="px-4 py-2" @click.stop v-if="resource.scopes">
            <div
            ...
            >
              <template slot="button-content" class="dropdown-toggle">
                <span class="d-none d-sm-inline"
                  >Can {{ permission.scopes.join(", ") }}</span
                >
              </template>
              ...
              <b-dropdown-item
                ><span
                  ...
                  >{{ $t("pages.uma.resources.unshare") }}</span
                ></b-dropdown-item
              >
            </b-dropdown>
          </div>
    ...
    <div slot="modal-footer" class="w-100">
      <div>
        <b-button type="button" variant="link" @click="unshareAll">{{
        ...
        <div class="float-right" v-if="newShare">
          <b-button
            ...
            variant="outline-secondary"
            @click="hideModal"
            >{{ $t("pages.uma.resources.cancel") }}</b-button
          >
          <b-button type="button" variant="primary" @click="validateResource">{{
  ...
  methods: {
    ...
    validateResource() {
      ...
      if (this.resource.policy && this.resource.policy.permissions) {
        let index = _.findIndex(this.resource.policy.permissions, (perm) => {
          return perm.subject === this.newShare;
        });
        ...
        if (index === -1) {
          if (this.newShare) {
            ...
            this.displayNotification(
              "error",
              this.$t("pages.uma.resources.noRequestingParty")
            );
          }
          ...
          this.displayNotification(
            "error",
            this.$t("pages.uma.resources.sameShareError", {
              requestingParty: this.newShare,
            })
          );
          this.resetModal();
        }
