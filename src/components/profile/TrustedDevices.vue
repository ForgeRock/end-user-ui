<template>
  ...
  >
      ...
      >
        <div slot="list-item-header" class="d-inline-flex w-100">
          <div class="d-flex mr-3 align-self-top">
            <fr-fallback-image
              :src="device.logo_uri"
              fallback="fa-television m-auto pt-1 pb-1"
            ></fr-fallback-image>
          </div>
    ...
    >
      ...
      {{
        $t("pages.profile.trustedDevices.removeConfirmation", {
    ...
    loadData() {
      ...
      let userName = this.$root.userStore.state.userName,
        ...
        url =
          ...
          userName +
          this.$root.applicationStore.state.amDataEndpoints.trustedDevices +
          query;
      ...
      selfServiceInstance
        ...
        .then(() => {
          this.displayNotification(
            "success",
            this.$t("pages.profile.trustedDevices.removeSuccess", {
              deviceName: this.confirmDevice.name,
            })
          );
          this.loadData();
