        ...
        <b-tabs>
          <b-tab :title="$t('pages.profile.settings')" active>
            <fr-account-security
              ...
              @updateKBA="updateKBA"
            ></fr-account-security>
            <fr-social-identities
              v-if="$root.userStore.state.internalUser === false"
              :clientToken="clientToken"
              :linkedProvider="linkedProvider"
            ></fr-social-identities>
            <fr-authorized-applications
              v-if="
                $root.applicationStore.state.amDataEndpoints &&
                $root.userStore.state.internalUser === false
              "
            ></fr-authorized-applications>
            <fr-trusted-devices
              v-if="
            ...
            <fr-consent
              v-if="$root.userStore.state.internalUser === false"
              :consentedMappings="profile.consentedMappings"
              @updateProfile="updateProfile"
            ></fr-consent>
...
export default {
  ...
  computed: {
    fullName() {
      ...
      if (
        this.$root.userStore.state.givenName.length > 0 ||
        this.$root.userStore.state.sn.length > 0
      ) {
        fullName = _.startCase(
          this.$root.userStore.state.givenName +
            " " +
            this.$root.userStore.state.sn
        );
      } else {
  ...
  methods: {
    updateProfile(payload, config = {}) {
      this.makeUpdateRequest(
        this.$root.userStore.state.managedResource,
        payload,
        config
      );
    },
    ...
    makeUpdateRequest(endpoint, payload, config = {}) {
      ...
      let successMsg =
          config.successMsg || this.$t("common.user.profile.updateSuccess"),
