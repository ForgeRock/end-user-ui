        ...
        <b-row>
          <b-col sm="8">
            <fieldset v-for="(select, id) in selected" :key="id" class="pb-3">
              <label
                >{{ $t("common.user.kba.question") }} {{ select.index }}</label
              >
              <b-form-select
                class="mb-3"
              ...
              >
                ...
                <b-form-input
                  ...
                  :name="
                    $t('pages.profile.accountSecurity.custom') + select.index
                  "
                  :class="[
                    {
                      'is-invalid': errors.has(
                        $t('pages.profile.accountSecurity.custom') +
                          select.index
                      ),
                    },
                    'form-control',
              ...
              <div class="form-group mb-0">
                ...
                <b-form-input
                  ...
                  :class="[
                    {
                      'is-invalid': errors.has(
                        $t('common.user.kba.answer') + select.index
                      ),
                    },
                    'form-control',
                  ]"
                ...
                <fr-validation-error
                  :validatorErrors="errors"
                  :fieldName="$t('common.user.kba.answer') + select.index"
                ></fr-validation-error>
              </div>
  ...
  methods: {
    initializeForm(minimumRequired) {
      ...
      this.selectOptions = _.map(this.questions, (question, key) => {
        return {
          value: key,
          text: question[locale] || question[fallbackLocale],
          disabled: true,
        };
      });
      ...
      this.selectOptions.unshift({
        value: null,
        text: this.$t("common.user.kba.selectQuestion"),
        disabled: true,
      });
      this.selectOptions.push({
        value: this.customIndex,
        text: this.$t("common.user.kba.custom"),
        disabled: false,
      });
    },
