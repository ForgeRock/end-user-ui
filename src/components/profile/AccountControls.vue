  ...
  <div>
      ...
      >
        <div slot="list-item-header" class="d-inline-flex w-100">
          ...
          <a
            ...
            @click.prevent
            href="#"
            >{{ $t("pages.profile.accountControls.downloadLink") }}</a
          >
        </div>
      </fr-list-item>
      <fr-list-item
      ...
      >
        <div slot="list-item-header" class="d-inline-flex w-100">
          ...
          <a
            ...
            @click.prevent
            href="#"
            >{{ $t("pages.profile.accountControls.deleteAccount") }}</a
          >
        </div>
      </fr-list-item>
    </fr-list-group>
    ...
    >
      ...
      <b-container>
        ...
        <hr />
        <p>
          {{ $t("pages.profile.accountControls.deleteModalDownload1") }}
          <a @click.prevent="downloadAccount" href="#">{{
            $t("pages.profile.accountControls.deleteModalDownload2")
          }}</a>
          {{ $t("pages.profile.accountControls.deleteModalDownload3") }}
        </p>
        <hr />
        <h5>
          {{ $t("pages.profile.accountControls.deleteModalContentList") }}
        </h5>
        <ul>
          <li>
            {{ $t("pages.profile.accountControls.deleteModalContentListItem") }}
          </li>
        </ul>
        <div class="custom-control custom-checkbox">
          ...
          />
          <label class="custom-control-label" for="confirmDeleteCheck">{{
      ...
      <div slot="modal-footer" class="w-100">
        <div class="float-right">
          ...
          <b-btn
            ...
            variant="danger"
            @click="deleteAccount"
            >{{ $t("pages.profile.accountControls.deleteModalButton") }}</b-btn
          >
        </div>
      </div>
    </b-modal>
...
export default {
  ...
  methods: {
    deleteAccount() {
      ...
      selfServiceInstance
        .delete(
          `/${this.$root.userStore.state.managedResource}/${this.$root.userStore.state.userId}`
        )
        .then(() => {
          this.$refs.deleteModal.hide();
          this.displayNotification(
            "success",
            this.$t("pages.profile.accountControls.deleteAccountSuccessful")
          );
          this.logoutUser();
        });
    },
    downloadAccount() {
      ...
      selfServiceInstance
        .get(
          `/${this.$root.userStore.state.managedResource}/${this.$root.userStore.state.userId}?_fields=*,idps/*,_meta/createDate,_meta/lastChanged,_meta/termsAccepted,_meta/loginCount`,
          []
        )
        .then((result) => {
          let data,
          ...
          if (navigator.msSaveBlob) {
            return navigator.msSaveBlob(
              new Blob([data], { type: "data:application/json" }),
              downloadName
            );
          } else {
...
@import "../../assets/scss/_full-screen-modal";
</style>
