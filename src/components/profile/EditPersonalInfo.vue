          ...
          >
            <template v-for="(field, index) in formFields">
              ...
              >
                ...
                <small v-if="!field.required" class="text-muted ml-1">{{
                ...
                <input
                  ...
                  :type="field.type"
                  :class="[
                    { 'is-invalid': errors.has(field.name) },
                    'form-control',
                  ]"
              ...
              <b-form-group :key="index" v-if="field.type === 'boolean'">
                <div class="d-flex flex-column">
                  <label class="mr-auto" :for="field.title">{{
                    field.title
                  }}</label>
    ...
    <div slot="modal-footer" class="w-100">
      <div class="float-right">
        ...
        <b-btn
          ...
          @click="saveForm"
          >{{ $t("common.form.saveChanges") }}</b-btn
        >
      </div>
    </div>
  </b-modal>
    ...
    saveForm() {
      ...
      this.isValid().then((valid) => {
        if (valid) {
          ...
          idmInstance
            .post(
              `policy/${this.$root.userStore.state.managedResource}/${this.$root.userStore.state.userId}?_action=validateObject`,
              policyFields
            )
            .then((policyResult) => {
              // reject any failedPolicyRequirements on properties that don't exist in this.formFields
              policyResult.data.failedPolicyRequirements = _.remove(
                policyResult.data.failedPolicyRequirements,
                (policy) =>
                  !_.map(this.formFields, "name").includes(policy.property)
              );

              if (policyResult.data.failedPolicyRequirements.length === 0) {
                this.$emit(
                  "updateProfile",
                  this.generateUpdatePatch(
                ...
                if (generatedErrors.length > 0) {
                  ...
                  this.displayNotification(
                    "error",
                    this.$t("pages.profile.editProfile.failedProfileSave")
                  );
                }
              }
            });
