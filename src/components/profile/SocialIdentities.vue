    ...
    <div v-if="providers.length > 0">
        <fr-list-group :title="$t('pages.profile.socialSignIn.title')" :subtitle="$t('pages.profile.socialSignIn.subtitle')">
            ...
            >
                <div slot="list-item-header" class="d-inline-flex w-100">
                    <div class="flex-grow-1">
                        ...
                        <span v-else class="fr-social-icon mr-3 text-center" :style="`background: ${provider.uiConfig.iconBackground}; color:${provider.uiConfig.iconFontColor};`">
                            <i v-if="provider.uiConfig.iconClass === 'fa-cloud'" :class="['fa', provider.uiConfig.iconClass]" />
                            <i v-else :class="['fab', provider.uiConfig.iconClass]" />
                        </span>
                    ...
                    <div class="float-right d-flex">
                        ...
                        <div v-if="provider._id" class="caret d-flex ml-3">
                ...
                <div v-if="provider._id" slot="list-item-collapse-body">
                    <fr-social-identity-panel :provider="provider" />
                </div>
            </fr-list-item>
        </fr-list-group>
...
<script>
import { each, extend, find, findIndex, get, includes, isUndefined } from "lodash";
...
export default {
    ...
    "computed": {
        disconnectedProvider () {
            const providerName = get(this.toDisconnect, "uiConfig.buttonDisplayName");
            return providerName || "";
        }
    },
    data () {
    ...
    // eslint-disable-next-line sort-keys
    created () {
        if (!isUndefined(this.clientToken)) {
            ...
            socialInstance.post(`${this.$root.userStore.state.managedResource}/${this.$root.userStore.state.userId}?_action=bind&provider=${this.linkedProvider}`, JSON.stringify(this.clientToken)).
                then(() => {
                    ...
                    this.displayNotification("success", `${find(this.providers, { "provider": this.linkedProvider }).uiConfig.buttonDisplayName} has been linked`);
    ...
    "methods": {
        ...
        disconnectProvider () {
            ...
            socialInstance.post(`${this.$root.userStore.state.managedResource}/${this.$root.userStore.state.userId}?_action=unbind&provider=${this.toDisconnect.provider}`).
                then(() => {
                    this.connectedProviders.splice(findIndex(this.connectedProviders, { "provider": this.toDisconnect.provider }), 1);
                    this.providers = this.setProviders();
                    this.displayNotification("success", `${find(this.providers, { "provider": this.toDisconnect.provider }).uiConfig.buttonDisplayName} has been unlinked`);
                    this.hideModal();
                }).
                catch((error) => {
        ...
        setProviders () {
            const providers = [];
            each(this.allAvailableProviders, (provider) => {
                let matchFound = false;

                each(this.connectedProviders, (idp) => {
                    // eslint-disable-next-line no-underscore-dangle
                    if (includes(idp._refResourceCollection, provider.provider)) {
                        matchFound = true;
                        providers.push(extend(idp, provider));
                    }
                });
        ...
        showModal (provider) {
            this.toDisconnect = find(this.providers, { provider });
