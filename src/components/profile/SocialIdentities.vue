<template>
  <div v-if="this.providers.length > 0">
    <fr-list-group
      :title="$t('pages.profile.socialSignIn.title')"
      :subtitle="$t('pages.profile.socialSignIn.subtitle')"
    >
      <fr-list-item
      ...
      >
        <div slot="list-item-header" class="d-inline-flex w-100">
          <div class="flex-grow-1">
            ...
            <span
              v-else
              class="fr-social-icon mr-3 text-center"
              :style="`background: ${provider.uiConfig.iconBackground}; color:${provider.uiConfig.iconFontColor};`"
            >
          ...
          <div class="float-right d-flex">
            <a class="align-self-center flex-grow-2 text-right" href="#">
              <span
                v-if="provider._refResourceId"
                @click.stop.prevent="showModal(provider.provider)"
                >{{ $t("pages.profile.socialSignIn.disconnect") }}</span
              >
              <span
                v-else
                @click.stop.prevent="connectProvider(provider.provider)"
                >{{ $t("pages.profile.socialSignIn.connect") }}</span
              >
            </a>
            <div class="caret d-flex ml-3" v-if="provider._id">
              <i
                class="fa fa-angle-down fa-lg pr-0 caret-down align-self-center"
              ></i>
              <i
                class="fa fa-angle-up fa-lg pr-0 pt-1 caret-up align-self-center"
              ></i>
            </div>
          </div>
        </div>
    ...
    >
      <div slot="modal-header" class="d-flex w-100 h-100">
        <h5 class="modal-title align-self-center text-center">
          {{ $t("pages.profile.socialSignIn.disconnect") }}
      ...
      <b-container>
        {{
          $t("pages.profile.socialSignIn.disconnectConfirmMsg", {
      ...
      <div slot="modal-footer" class="w-100">
        <div class="float-right">
          <b-btn variant="outline-secondary" @click="hideModal">{{
            $t("common.form.close")
          }}</b-btn>
          <b-btn type="button" variant="danger" @click="disconnectProvider">{{
            $t("pages.profile.socialSignIn.disconnect")
          }}</b-btn>
        </div>
      </div>
...
export default {
  ...
  created() {
    if (!_.isUndefined(this.clientToken)) {
      ...
      socialInstance
        .post(
          `${this.$root.userStore.state.managedResource}/${this.$root.userStore.state.userId}?_action=bind&provider=${this.linkedProvider}`,
          JSON.stringify(this.clientToken)
        )
        .then(() => {
          this.getAllConnectedProviders(
          ...
          this.displayNotification(
            "success",
            `${
              _.find(this.providers, { provider: this.linkedProvider }).uiConfig
                .buttonDisplayName
            } has been linked`
          );
  ...
  methods: {
    ...
    getAllConnectedProviders(socialInstance) {
      ...
      socialInstance
        .get(
          `${this.$root.userStore.state.managedResource}/${this.$root.userStore.state.userId}?_fields=idps/*`
        )
        .then((response) => {
    ...
    connectProvider(provider) {
      ...
      /* istanbul ignore next */
      socialInstance
        .post("/identityProviders?_action=getAuthRedirect", {
          provider: provider,
    ...
    disconnectProvider() {
      ...
      socialInstance
        .post(
          `${this.$root.userStore.state.managedResource}/${this.$root.userStore.state.userId}?_action=unbind&provider=${this.toDisconnect.provider}`
        )
        .then(() => {
          this.connectedProviders.splice(
            _.findIndex(this.connectedProviders, {
              provider: this.toDisconnect.provider,
            }),
            1
          );
          this.providers = this.setProviders();
          this.displayNotification(
            "success",
            `${
              _.find(this.providers, { provider: this.toDisconnect.provider })
                .uiConfig.buttonDisplayName
            } has been unlinked`
          );
          this.hideModal();
        })
