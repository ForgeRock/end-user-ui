<template>
    <fr-list-group v-show="oauthApplications" :title="$t('pages.profile.oauthApplications.title')" :subtitle="$t('pages.profile.oauthApplications.subtitle')">
        <template v-if="oauthApplications.length > 0">
            ...
            >
                <div slot="list-item-header" class="d-inline-flex w-100">
                    ...
                    <a class="align-self-center flex-grow-2 text-right" href="#" @click.prevent="showConfirmationModal(application)">{{ $t('common.form.remove') }}</a>
                </div>
            </fr-list-item>
        </template>
        <template v-else>
        ...
        <b-modal id="authAppConfirmationModal" ref="fsModal" class="" cancel-variant="outline-secondary">
