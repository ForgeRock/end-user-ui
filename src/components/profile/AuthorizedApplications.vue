<template>
  ...
  >
        ...
        <div slot="list-item-header" class="d-inline-flex w-100">
          ...
          <div class="flex-grow-1">
            ...
            <small class="text-muted subtext"
              >{{ $t("pages.profile.oauthApplications.expires") }}
              {{ new Date(application.expiryDateTime).toUTCString() }}</small
            >
          </div>
          <a
    ...
    >
      ...
      {{
        $t("pages.profile.oauthApplications.removeConfirmation", {
          applicationName: confirmApplication.name,
        })
      }}
      <div slot="modal-footer">
        <div class="float-right">
          <b-btn variant="outline-secondary" @click="$refs.fsModal.hide()">{{
  ...
  methods: {
    loadData() {
      ...
      let userName = this.$root.userStore.state.userName,
        ...
        url =
          this.$root.applicationStore.state.amDataEndpoints.baseUrl +
          userName +
          this.$root.applicationStore.state.amDataEndpoints.oauthApplications +
          query;
