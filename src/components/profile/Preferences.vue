  ...
  methods: {
    loadData() {
      let keys = _.keys(this.$root.userStore.state.profile.preferences),
        preferences = _.cloneDeep(
          this.$root.userStore.state.schema.properties.preferences.properties
        );

      _.each(keys, (key) => {
        preferences[key].value =
          this.$root.userStore.state.profile.preferences[key];
    ...
    generatePatch(preference, value) {
      return [
        { operation: "replace", field: "/preferences/" + preference, value },
      ];
    },
