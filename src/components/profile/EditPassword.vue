  ...
  <fr-list-item :collapsible="true" :panelShown="false">
    <div slot="list-item-header" class="d-inline-flex w-100 media">
      ...
      <div class="d-flex ml-3 align-self-center">
        ...
        <div
          v-show="!showCancelButton"
          class="btn btn-sm btn-link float-right btn-edit"
          @click="showCancelButton = true"
        >
          {{ $t("common.form.reset") }}
        </div>
      </div>
        ...
        <b-row>
          <b-col sm="8">
            <b-form-group>
              ...
              <div class="form-label-password form-label-group mb-0">
                <b-form-input
                  ...
                  "
                  :class="[
                    { 'is-invalid': errors.has('currentPassword') },
                    'form-control',
                  ]"
                  :type="inputCurrent"
            ...
            <fr-password-policy-input
              :policyApi="`${this.$root.userStore.state.managedResource}/${userId}`"
              v-model="newPassword"
            >
            ...
            >
              {{ $t("pages.profile.accountSecurity.rememberPassword") }}
  ...
  methods: {
    ...
    onSavePassword() {
      const headers = {
          ...
          "X-OpenIDM-Reauth-Password": this.encodeRFC5987IfNecessary(
            this.currentPassword
          ),
        },
        payload = [
          { operation: "add", field: "/password", value: this.newPassword },
        ],
      ...
      this.$validator.validateAll().then((valid) => {
        if (valid) {
          ...
          this.displayNotification(
            "error",
            this.$t("pages.profile.accountSecurity.invalidPassword")
          );
        }
      });
    },
