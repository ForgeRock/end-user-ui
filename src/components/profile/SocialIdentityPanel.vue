      ...
      <div class="col-8">
        <div class="media">
          ...
          <div class="media-body">
            <strong class="fr-social-details-title d-block mb-0">{{
              profile.displayName
            }}</strong>
            <span v-show="accountDisplayName" class="text-muted">{{
              accountDisplayName
            }}</span>
          </div>
        </div>
...
export default {
  ...
  created() {
    ...
    this.getRequestService()
      .post(
        "identityProviders?_action=normalizeProfile",
        this.getProfileRequestPayload()
      )
      .then(({ data }) => {
  ...
  computed: {
    showAccountDetails() {
      let profileNotEmtpy = !_.isEmpty(this.profile),
        ...
        profileHasEmailOrUsername =
          _.has(this.profile, "email") || _.has(this.profile, "username");

      return (
        profileNotEmtpy && (profileHasDisplayName || profileHasEmailOrUsername)
      );
    },
    photoUrl() {
      return this.profile.photoUrl || "@/assets/images/profile-default.png";
    },
    accountDisplayName() {
