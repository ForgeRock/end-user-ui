...
describe("EditPassword.vue", () => {
  ...
  beforeEach(() => {
    ...
    sandbox.stub(EditPassword.methods, "validate").callsFake(() => {
  ...
  describe("#onSavePassword", () => {
    it.skip('should emit "patch" with payload and config', () => {
      const wrapper = shallowMount(EditPassword, {
        provide: () => ({
          $validator: v,
        }),
      ...
      return Vue.nextTick().then(() => {
        ...
        expect(config.headers)
          .to.have.property("X-OpenIDM-Reauth-Password")
          .that.equals("test current");
      });
    });
  });
});
