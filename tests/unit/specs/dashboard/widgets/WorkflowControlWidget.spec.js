...
describe("Workflow Control Widget Component", () => {
  ...
  describe("#getTaskParams", () => {
    it("should return the correct params object for task requests", () => {
      expect(WorkflowControl.methods.getTaskParams("testUserId"))
        .to.be.an("object")
        .and.to.include({ _queryId: "gettasksview" })
        .and.to.include({ userId: "testUserId" });

      expect(
        WorkflowControl.methods.getTaskParams("testUserId", "assignedTasks")
      )
        .to.be.an("object")
        .and.to.include({ _queryId: "gettasksview" })
        .and.to.include({ userId: "testUserId" })
  ...
  describe("#getTaskGroup", () => {
    it("should return the specified task group", () => {
      ...
      expect(wrapper.vm.getTaskGroup("assignedTasks")).to.equal(
        wrapper.vm.assignedTasks
      );
      expect(wrapper.vm.getTaskGroup("availableTasks")).to.equal(
        wrapper.vm.availableTasks
      );
    });
  });
  ...
  describe("toTasks", () => {
    ...
    it("should populate a method to fetch a process if no process defined", () => {
      ...
      expect(wrapper.vm.assignedTasks)
        .to.have.property("test")
        .that.includes({ name: "test name" })
        .and.that.has.property("process")
        .that.has.property("fetchProcessDefinition");
