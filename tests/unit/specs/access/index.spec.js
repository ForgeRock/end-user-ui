...
describe("ListResource.vue", () => {
  ...
  beforeEach(() => {
    ...
    sandbox.stub(ListResource, "mounted").callsFake(() => {});
  });
  ...
  it("Access resource URL generated", () => {
    ...
    expect(wrapper.vm.buildGridUrl("test", ["test"], ["test"], 2)).to.equal(
      "test/test?_queryFilter=test&_pageSize=10&_sortKeys=test&_fields=test&_pagedResultsOffset=10"
    );
  });
  ...
  it("Generated query filter for search", () => {
    ...
    expect(wrapper.vm.generateSearch("test", ["test1", "test2"])).to.equal(
      'test1+sw+"test"+OR+test2+sw+"test"'
    );
